...
$(function () {
    cgdetailid = getQueryString("cgdetailid");
    fjdanhao = getQueryString("fjdanhao");
    spxxitemno = getQueryString("itemno");
    var newspxx = getQueryString("newspxx");
    var spcode = getQueryString("spcode");
    cgtradeid= getQueryString("cgtradeid");
    $("#fjdanhao").textbox("setValue", fjdanhao);
    if(newspxx == 1){
        newSpxxFun();
    }else{
        $.ajax({
            url: pageContext + "/cgfjController/getSpxxBySelective",
            data: {"cgdetailid": cgdetailid,"spcode":spcode},
            type: "POST",
            dataType: "json",
            async: false,
            success: function (data) {
                if (data.success) {
                    ...
                    lbid = data.obj.lbid;
                    dwid = data.obj.dwid;
                    spxxitemno != '' ? $("#itemno").textbox("setValue", spxxitemno) : $("#itemno").textbox("setValue", data.obj.itemno);
                    ...
                    $('#spxxtradeid').combobox('select',cgtradeid);
                } else if (data.msg == 'NOTINFO') {
                    newSpxxFun();
                } else {
                    alert(data.msg);
                    offThisWindows();
...
function newSpxxFun() {
    ...
    $("#itemno").textbox("setValue", spxxitemno);
    $('#spxxtradeid').combobox();
    $('#spxxtradeid').combobox('select', cgtradeid);
}
...
var formData = function () {
    ...
    for (var i = 0; i < selectData.total; i++) {
        kpiList.push({
            ...
            "empname": selectData.rows[i].ygxm,
            "fjsl": selectData.rows[i].fjsl
        })
    }
