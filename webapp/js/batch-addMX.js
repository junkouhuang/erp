...
var saveMx = function () {
    ...
    var rows = data.rows;
    var parentRows = window.parent.getTable();
    if (rows.length == 0){
        var index = parent.layer.getFrameIndex(window.name);
        parent.layer.close(index);
        return false;
    ...
    $.each(rows, function(index1, item1){
        ...
        var s_cm = item1.cm;
        var is_varify = false;
        $.each(parentRows, function(index2, item2){
            var p_ys = item2.ys;
            var p_cm = item2.cm;
            if (s_ys == p_ys && s_cm == p_cm){
                is_varify = true;
                msg = "颜色为" + s_ys + "-尺码为" + s_cm + "已经存在，无法追加";
                mark_obj = item1;
                return false;
            }
        });
    ...
    if (msg != null){
        layer.msg(msg);
        var rowIndex = $('#dg').datagrid('getRowIndex', mark_obj);
    ...
    var batchcode = getQueryVariable("batchcode");
    if (batchcode == "" || batchcode == null || batchcode == undefined){
        layer.msg("批次号不确定，无法进行追加操作！");
        return false;
    }
    ...
    $.each(rows, function(index, item){
        var mxObj = {};
        mxObj.cgmxcode = batchcode + item.ysid + item.cmcode;    // 采购明细编号 = 采购商品编号 + 颜色序号 + 尺码编号
        mxObj.cgspcode = batchcode;
    ...
    layer.confirm('确定进行追加吗？', {
        btn: ['确定','取消'],
        shade: false,
        btn1:function(index){
            $.ajax({
                url: pageContext + "/spBatchController/batchAddMX/" + batchcode,
                type: "POST",
                contentType : 'application/json;charset=utf-8',
                async: false,
                dataType: "json",
