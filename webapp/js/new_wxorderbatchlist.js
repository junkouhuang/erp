...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        ...
        $('#table').bootstrapTable({  //表格ID
            ...
            pageSize: 10,                       //每页的记录行数（*）
            pageList: [10,  100, 1000],        //可供选择的每页的行数（*）
            idField: "id",
...
function queryParams(params) {
    ...
    var time = $("#time").val();
    var isPcCreateFhOrder = $("#isPcCreateFhOrder").val();
    ...
    var temp = {   //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
        ...
        time:time,
        isPcCreateFhOrder:isPcCreateFhOrder
    };
    return temp;
}
...
function addWxNoOrderFhorder() {
    ...
    for (var i = 0; i < optionSelections.length; i++) {
        if (optionSelections[0].fhid != null) {
            ...
            return;
        } else if (optionSelections[0].status != 3 || optionSelections[0].wxorders.paystatus != 3) {
            console.log(optionSelections[0].status + "   " + optionSelections[0].wxorders.paystatus);
            layer.msg("只有待支付状态的才能合成发货单！！", function () {
            });
            return;
        }
        if (i != 0) {
    ...
    layer.confirm('确定要生成发货单？没完工的批次会自动剔除..', {
        ...
        $.ajax({
            url: pageContext + "/wxOrderBatchController/createWxFhorder",
            type: "post",
...
function revokeWxOrderBatch() {
    ...
    for (var i = 0; i < selections.length; i++) {
        if (selections[i].status > 3) {
            layer.msg("只有待发货状态的订单批次才能撤销，订单号：" + selections[i].wxorders.ordercode + ",批次号："
                + selections[i].batchcode + " 不符合要求！！", function () {});
            return;
        }
        if (selections[i].wxjhprint) {
            layer.msg("只有未拣货的订单批次才能撤销，订单号：" + selections[i].wxorders.ordercode + ",批次号："
                + selections[i].batchcode + " 不符合要求！！", function () {});
            return;
        }
    ...
    $.ajax({
        url: pageContext + "/wxOrderBatchController/revokeWxOrderBatchs/1",
        type: "post",
        data: JSON.stringify(wxorderidList),
        contentType: 'application/json;charset=UTF-8',
        dataType: "json",
...
function addWxNoOrderFhorders() {
    ...
    for (var i = 0; i < optionSelections.length; i++) {
        if (optionSelections[0].fhid != null) {
            ...
            console.log(optionSelections[0].status + "   " + optionSelections[0].wxorders.paystatus);
            layer.msg("只有支付状态的訂單才能合成发货单！！", function () {});
            return;
        }
        if (optionSelections[i].baudied !=0) {
            noHlist += optionSelections[i].batchcode + ',';
        }
        wxorderidList.push(optionSelections[i].id);
    }
    ...
    layer.confirm('确定要生成发货单？', {
        btn: ['确定', '取消']
    }, function (index) {
        $.ajax({
            url: pageContext + "/wxOrderBatchController/createWxFhorders",
            type: "post",
            data: JSON.stringify(wxorderidList),
            contentType: 'application/json;charset=UTF-8',
            dataType: "json",
...
function exportWxorderBatchInfo() {
    var isCreateFhorder = $("#isCreateFhorder").val();
    var time = $("#time").val();
    var batchcode = $("#batchcode").val();
    var isPcCreateFhOrder = $("#isPcCreateFhOrder").val();
    if(batchcode == null || batchcode == undefined || batchcode == ""){
        layer.alert("批次号不能为空！！");
        return;
    }
    window.location.href=pageContext+"/reportController/exportWxorderBatchInfo/"+batchcode+"?time="+time+"&isCreateFhorder="+isCreateFhorder+"&isPcCreateFhOrder="+isPcCreateFhOrder;
}